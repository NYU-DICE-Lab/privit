...
parser = argparse.ArgumentParser(description="PriViT Training")
parser.add_argument('outdir', type=str, help='folder to save model and training log')
parser.add_argument('--dataset', default='cifar10', type=str)
parser.add_argument('--workers', default=4, type=int, metavar='N',
                    help='number of data loading workers (default: 4)')
parser.add_argument('--finetune_epochs', default=100, type=int,
                    help='number of total epochs for final finetuning')
...
parser.add_argument('--gelu_budget', default=0, type=int, help='Number of Gelus that are retained by the model')
parser.add_argument('--softmax_budget', default=0, type=int,help='Number of Softmax that are retained by the model')
...
parser.add_argument('--skip', action='store_true', help='Flag to turn off teacher finetuning')
parser.add_argument('--skip_softmax', action='store_true', help='Flag to turn off parametric mask from softmax')
parser.add_argument('--skip_gelu', action='store_true', help='Flag to turn off parametric mask from gelu')
parser.add_argument('--model_type', '-mt', help='ViT model to use', default='WinKawaks/vit-tiny-patch16-224', choices=['google/vit-base-patch16-224-in21k', 'google/vit-large-patch16-224-in21k','WinKawaks/vit-small-patch16-224','WinKawaks/vit-tiny-patch16-224'])
parser.add_argument('--aa', action='store_true', default=False, help='Enable RandAugment')
parser.add_argument('--cutmix_prob', type=float, default=0.5, help='Probability of applying CutMix augment strategy')
...
def main(args):
    ...
    train_dataset = get_dataset(args.dataset, 'train', args.aa)
    ...
    # defining the teacher (fintuning the imagenet pre-trained model)
    model = CustomViTForImageClassification.from_pretrained(args.model_type,
    ...
    finetune_epoch = 1 if args.debug else 10 # initial finetuning of teacher model
    if not args.skip:
    ...
    # Deepcopy the model and apply the parametric masks on gelus and softmax, and start taylorizing this student model
    ...
    total_gelu = gelu_counting(net, args)

    lowest_softmax_count, softmax_count = total_softmax, total_softmax
    lowest_gelu_count, gelu_count = total_gelu, total_gelu
    nonlinear_count = softmax_count + gelu_count
    ...
    for epoch in range(args.epochs):
        ...
        gelu_count = gelu_counting(net, args)
        nonlinear_count = softmax_count + gelu_count
        
        # flags to increase lasso coefficient
        penalty_flag_softmax = lowest_softmax_count - softmax_count <200  and not frozen_softmax 
        penalty_flag_gelus = lowest_gelu_count - gelu_count <1  and not frozen_gelu
        ...
        if gelu_count < lowest_gelu_count:
            lowest_gelu_count = gelu_count
        
        # flags to check if gelu/softmax budget is achieved
        freeze_gelu = gelu_count <=args.gelu_budget
        freeze_softmax = softmax_count <=args.softmax_budget
        
        if freeze_gelu and not frozen_gelu:
            log(logfilename,"Freezing Gelus")
            print("GELU Count: {:}".format(gelu_count))
            frozen_gelu = True
        ...
        # flag to exit the joint optimization of network and parametric masks
        break_flag = frozen_softmax and frozen_gelu
        if break_flag:
    ...
    # Binarize the learned parametric mask and freeze them
    for name, param in net.named_parameters():
        if 'alpha' in name:
            boolean_list = param.data > args.threshold
            param.data = boolean_list.float()
    ...
    finetune_epoch = 10 if args.debug else args.finetune_epochs # finetune epochs for final training of the student network
    optimizer = torch.optim.AdamW(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, finetune_epoch)

    log(logfilename, "Finetuning the taylorized model")
